    ...
    unsigned char** new_class_data)
{
    ...
    std::string event_type_log = class_being_redefined != nullptr ? "Retransform/Redefine" : "Initial Load";
    std::string common_log_part = ": Class: " + log_name_str + " (Length: " + std::to_string(class_data_len) + " bytes)";

    if (local_instance) {
        local_instance->logger("ClassFileLoadHook [" + event_type_log + "]" + common_log_part);
    }
    else {
        std::cerr << "[ClassFileLoadHook] [" << event_type_log << "]" << common_log_part << " (instance is null!)" << std::endl;
    }
...
namespace hook {
    ...
    bool c_hook::attach() {
        ...
        if (!JNI_GetCreatedJavaVMs_ptr) {
            logger("Error getting JNI_GetCreatedJavaVMs function");
            return false;
        }
    ...
    bool c_hook::setup_jvmti_hooks() {
        if (!m_jvmti_env) {
            logger("setup_jvmti_hooks: Error - m_jvmti_env is NULL!");
            return false;
        }

        if (m_jvmti_hook_active) {
            logger("JVMTI hooks already active.");
            return true;
        }
    ...
    bool c_hook::trigger_retransformation_for_all_classes() {
        ...
        err = m_jvmti_env->RetransformClasses(count_to_retransform, classes_to_retransform);
        ...
        if (err == JVMTI_ERROR_NONE) {
            ...
            case JVMTI_ERROR_UNMODIFIABLE_CLASS: logger("JVMTI_ERROR_UNMODIFIABLE_CLASS."); return true;
